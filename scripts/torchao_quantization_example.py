...
# make a couple of deep copies
model_w16a16 = copy.deepcopy(example_base_model)
model_w8a8 = copy.deepcopy(example_base_model)

# naively quantize the first model
model_w16a16 = model_w16a16.to(dtype=torch.bfloat16)

# quantize the second model with torchao to int8
...
torch.save(model_w16a16.state_dict(), "scripts/model_w16a16.pth")
torch.save(model_w8a8.state_dict(), "scripts/model_w8a8.pth")

# model sizes
original_size = os.path.getsize("scripts/base_model.pth")
size_16_bit = os.path.getsize("scripts/model_w16a16.pth")
size_8_bit = os.path.getsize("scripts/model_w8a8.pth")

print(f"Size reduction, 16 bit: {original_size / size_16_bit}")
...
if torch.cuda.is_available():
    ...
    model_w16a16.to(device=torch.device("cuda")).eval()
    ...
    w16a16_model_inference = np.zeros(shape=(1000,))
    ...
    for _ in range(10):
        ...
        _ = model_w16a16(example_inputs.to(dtype=torch.bfloat16))
        _ = model_w8a8(example_inputs)
        
    torch.cuda.synchronize()
    for i in range(1000):
        example_inputs = torch.rand(size=(128, 81), device=torch.device("cuda"), dtype=torch.float32)
        start = time.time()
        _ = example_base_model(example_inputs)
        end = time.time()
        duration = end - start
    ...
    torch.cuda.synchronize()
    for i in range(1000):
        example_inputs = torch.rand(size=(128, 81), device=torch.device("cuda"), dtype=torch.bfloat16)
        start = time.time()
        _ = model_w16a16(example_inputs)
        end = time.time()
        duration = end - start
        w16a16_model_inference[i] = duration
        
    w16a16_inference_df = pd.DataFrame(data={"inference_time": w16a16_model_inference})
    w16a16_inference_df = w16a16_inference_df.assign(model="w16a16")
    ...
    inference_df = pd.concat([base_inference_df, w16a16_inference_df, w8a8_inference_df], axis=0)
    
    figure = px.box(data_frame=inference_df, x="model", y="inference_time", color="model")
