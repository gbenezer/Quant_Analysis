...
# result df lists
full_result_dfs = [
    pd.read_csv(result_path, index_col=0).dropna().assign(experiment_number=(experiment+1))
...
full_result_df["model_ID"] = full_result_df["model_ID"] + "_" + full_result_df["experiment_number"].astype(str)
weight_only_df = pd.concat(weight_result_dfs)
weight_only_df["eval_run"] = weight_only_df["eval_run"].astype("int")
weight_only_df["model_ID"] = weight_only_df["model_ID"] + "_" + weight_only_df["experiment_number"].astype(str)

# saving the files
full_result_df.to_csv((OUTPUT_DATA_PATH / "full_ptq_config_sampling_experiments.csv"))
weight_only_df.to_csv((OUTPUT_DATA_PATH / "weight_only_ptq_config_sampling_experiments.csv"))